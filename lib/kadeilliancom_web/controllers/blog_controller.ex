defmodule KadeilliancomWeb.BlogController do
  ...
  def index(conn, _params) do
    blogs = Work.list_blogs()
    render(conn, "index.html", blogs: blogs)
  end

  def new(conn, _params) do
    changeset = Work.change_blog(%Blog{})
    render(conn, "new.html", changeset: changeset)
  end

  def create(conn, %{"blog" => blog_params}) do
    case Work.create_blog(blog_params) do
      {:ok, blog} ->
        conn
  ...
  def update(conn, %{"id" => id, "blog" => blog_params}) do
    ...
    case Work.update_blog(blog, blog_params) do
      {:ok, blog} ->
        conn
  ...
  def delete(conn, %{"id" => id}) do
    blog = Work.get_blog!(id)
    {:ok, _blog} = Work.delete_blog(blog)
